...
@property (strong, nonatomic) IBOutlet UIView *adjustView;
@property (strong, nonatomic) IBOutlet UIImageView *sourceImageView;
@property (strong, nonatomic) NSTimer *frameDetectionTimer;
@end
...
//    _sourceImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, self.adjustView.bounds.size.width, self.adjustView.bounds.size.height)];
...
//    [self.adjustView addSubview:_sourceImageView];
//    UILabel * labelTitle = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, self.view.bounds.size.width, 80)];
//    labelTitle.text = @"Crop Image";
...
//    [self.view addSubview:labelTitle];
    ...
    [self.adjustView addSubview:_adjustRect];
    
    _adjustToolBar = [[UIToolbar alloc] initWithFrame:CGRectMake(0, self.adjustView.bounds.size.height - kCameraToolBarHeight, self.adjustView.bounds.size.width, kCameraToolBarHeight)];
    ...
    [self.adjustView addSubview:_adjustToolBar];
    _adjustToolBar.hidden = true;
    [self detectEdges];
    _frameDetectionTimer = [NSTimer scheduledTimerWithTimeInterval:0.10 target:self selector:@selector(frameMoveDetectionCheck) userInfo:nil repeats:true];
}
...
- (IBAction)resetFrameAction:(id)sender {
    [self resetRectFrame];
...
- (void) frameMoveDetectionCheck
{
    if ([_adjustRect frameEdited])
    {
        _resetFremeButton.hidden = false;
    }
    else
    {
        _resetFremeButton.hidden = true;
    }
}
